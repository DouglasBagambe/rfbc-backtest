...
| Resumption | Each pair/cross unit is atomically written with a SHA-256 completion manifest. Invalid, truncated or checksum-mismatched checkpoints cannot be resumed or finalized. Finalization reads each valid checkpoint once, marks each fully partitioned source atomically, then summarizes one edge-level partition at a time; an interrupted finalizer resumes only completed partition sources and never retains the full checkpoint universe or repeats a per-group scan in RAM. |
