...
def main() -> None:
    ...
    while RUNNING:
        try:
            ...
            failures+=1
            fx101.log("worker_failure", error=type(exc).__name__, failures=failures)
            if failures in (1,5,20):
                fx101.telegram_send(f"Fx101 worker warning: {type(exc).__name__}; retrying safely.")
        time.sleep(poll)
